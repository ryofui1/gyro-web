...
function randInt(a,b) {
    return Math.floor(rand(a,b+1-0.0000001));
}
...
function tick(now) {
    ...
    for (let i = 0; i < petals.length; i++) {
        ...
        if (
            ...
            petals[i] = newPetal;
            const newPetal2 = generatePetal();
            if (wind > 30) {
                newPetal2.x = rand(-30, innerWidth * 0.2);
            }
            if (wind < -30) {
                newPetal2.x = rand(innerWidth * 0.8, innerWidth + 30);
            }
            petals.push(newPetal2);
        }
    }
    drawPetals(petals);
    requestAnimationFrame(tick); // 毎tick行うという設定
}
